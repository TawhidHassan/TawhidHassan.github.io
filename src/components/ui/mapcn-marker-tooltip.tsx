  ...
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/50 backdrop-blur-xs">
      <div className="flex gap-1">
        ...
        <span className="size-1.5 animate-pulse rounded-full bg-muted/60 [animation-delay:150ms]" />
        <span className="size-1.5 animate-pulse rounded-full bg-muted/60 [animation-delay:300ms]" />
      </div>
    </div>
  );
}
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div className={cn("pointer-events-none rounded-md bg-foreground px-2 py-1 text-xs text-balance text-background shadow-md", className)}>
      {children}
    </div>,
    container,
  );
